...
def run(
    ...
    """Build the rear ear v2 from the original ear bodies."""
    ...
    rib_center = adsk.core.Point3D.create(
        BOSS_X * MM,
        (RIB_Y_MIN + RIB_Y_MAX) / 2 * MM,
        (RIB_Z_START + RIB_Z_END) / 2 * MM,
    )
    rib = temp_mgr.createBox(
        adsk.core.OrientedBoundingBox3D.create(
            rib_center,
            adsk.core.Vector3D.create(1, 0, 0),
            adsk.core.Vector3D.create(0, 1, 0),
            RIB_WIDTH * MM,
            (RIB_Y_MAX - RIB_Y_MIN) * MM,
            (RIB_Z_END - RIB_Z_START) * MM,
        )
    )
    temp_mgr.booleanOperation(combined, rib, adsk.fusion.BooleanTypes.UnionBooleanType)

    def slab(x0_mm, x1_mm, y0_mm, y1_mm, z0_mm, z1_mm):
        center = adsk.core.Point3D.create(
            (x0_mm + x1_mm) / 2 * MM, (y0_mm + y1_mm) / 2 * MM, (z0_mm + z1_mm) / 2 * MM
        )
        return temp_mgr.createBox(
            adsk.core.OrientedBoundingBox3D.create(
                center,
                adsk.core.Vector3D.create(1, 0, 0),
                adsk.core.Vector3D.create(0, 1, 0),
    ...
    tangency_relief = slab(
        TANGENCY_X0, TANGENCY_X1, TANGENCY_Y0, TANGENCY_Y1, -1.0, 67.0
    )
    temp_mgr.booleanOperation(
        combined, tangency_relief, adsk.fusion.BooleanTypes.DifferenceBooleanType
    )

    side_wall = slab(
        SIDE_WALL_X0, SIDE_WALL_X1, SIDE_WALL_Y0, SIDE_WALL_Y1, SIDE_WALL_Z0, RIB_Z_END
    )
    temp_mgr.booleanOperation(
    ...
    if DUCT_GROOVE:
        ...
        for y0, y1 in (
            (RIB_Y_MIN, RIB_Y_MIN + RAIL_BLOCK_HEIGHT),
        ...
        for y0, y1 in (
            (GROOVE_BOTTOM_Y0, GROOVE_BOTTOM_Y1),
    ...
    for boss_y in BOSS_ROWS:
        if NUT_TRAP:
            screw_hole = cylinder(
                BOSS_X, boss_y, SCREW_RELIEF_Z0, RIB_Z_END + 0.1, SCREW_CLEAR_DIA
            )
            temp_mgr.booleanOperation(
                combined, screw_hole, adsk.fusion.BooleanTypes.DifferenceBooleanType
            )
            nut_slot = slab(
                NUT_SLOT_X0,
                NUT_SLOT_X1,
            ...
            temp_mgr.booleanOperation(
                combined, nut_slot, adsk.fusion.BooleanTypes.DifferenceBooleanType
            )
        else:
            pocket = cylinder(
                BOSS_X,
                boss_y,
                RIB_Z_END - INSERT_HOLE_DEPTH,
                RIB_Z_END + 0.1,
                INSERT_HOLE_DIA,
            )
            temp_mgr.booleanOperation(
    ...
    width = (bb.maxPoint.x - bb.minPoint.x) * 10
    height = (bb.maxPoint.y - bb.minPoint.y) * 10
    depth = (bb.maxPoint.z - bb.minPoint.z) * 10
    print(
        f"Created document '{new_doc.name}' with body '{new_body.name}': "
        f"{width:.2f} x {height:.2f} x {depth:.2f} mm, {new_body.faces.count} faces"
    )
